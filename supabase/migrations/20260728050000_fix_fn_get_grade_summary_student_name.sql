-- Fix fn_get_grade_summary: tambah full_name dan nis siswa ke response
-- agar grid Nilai Akhir menampilkan nama, bukan UUID
        ...
        'students', (
            SELECT COALESCE(
                jsonb_agg(
                    ...
                    ) ORDER BY COALESCE(st.full_name, gs.student_id::text)
                ),
                '[]'::jsonb
            )
            FROM grade_summaries gs
            LEFT JOIN students st ON st.student_id = gs.student_id
            WHERE gs.school_id     = v_school_id
              AND gs.subject_id    = p_subject_id
              AND gs.class_id      = p_class_id
              AND gs.academic_year = p_academic_year
