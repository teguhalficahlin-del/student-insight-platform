-- ============================================================
-- Fix: Forum Sekolah PATH A tidak bisa kirim ke siswa/ortu
--
-- Root cause: filter role_type di PATH A hanya mengizinkan staf.
-- Ketika guru memilih "semua siswa kelas X" sebagai penerima,
-- semua user_id siswa (role_type='SISWA') dibuang → guard
-- array_length <= 1 → RAISE EXCEPTION → frontend tampilkan
-- pesan error generik.
--
-- Fix: tambah 'SISWA' dan 'ORTU' ke allowlist PATH A.
-- Cross-tenant isolation tetap dijaga oleh AND u.school_id = v_school_id.
-- ============================================================
