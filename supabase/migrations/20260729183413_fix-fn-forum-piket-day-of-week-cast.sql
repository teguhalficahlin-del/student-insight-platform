-- Fix branch GURU_PIKET:
-- duty_schedules.day_of_week adalah enum (SENIN..SABTU), bukan smallint.
-- Konversi p_day_of_week (smallint 1-6) → enum via CASE.
-- NULL p_day_of_week = semua hari (tidak filter hari).
...
BEGIN
  ...
  -- Konversi smallint → day_of_week enum (NULL = semua hari)
  ...
  ELSIF p_target_group = 'GURU_PIKET' THEN
    RETURN QUERY
      ...
      WHERE u.school_id    = v_school_id
        ...
        -- NULL = semua hari; integer → enum via v_day_enum
