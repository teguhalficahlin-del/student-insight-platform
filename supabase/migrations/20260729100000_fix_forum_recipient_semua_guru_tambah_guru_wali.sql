-- Fix Bug #1: SEMUA_GURU salah scope (memasukkan semua role staf)
-- Fix Bug #2: Tambah branch SEMUA_GURU_WALI (Guru Wali tidak ada sama sekali)
--
-- Bug #1: branch SEMUA_GURU sebelumnya filter
--   role_type IN ('GURU','BK','WAKA_KURIKULUM',...,'TU')  ← semua staf
-- Seharusnya: role_type = 'GURU' saja (identik dengan GURU_MAPEL)
--
-- Bug #2: Guru Wali diidentifikasi via tabel guru_wali_assignments.
-- Branch baru SEMUA_GURU_WALI mengembalikan distinct guru yang aktif
-- mengampu wali siswa pada academic_year berjalan.
...
BEGIN
  ...
  -- ── Bug #1 fix: hanya role_type = 'GURU' (bukan semua staf) ──────────
  ...
  -- GURU_MAPEL: picker individual, sama persis dengan SEMUA_GURU
  ...
  -- ── Bug #2 fix: tambah branch Guru Wali ──────────────────────────────
