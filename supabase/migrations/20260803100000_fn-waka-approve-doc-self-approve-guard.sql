-- Fix: tambah guard self-approve di fn_waka_approve_doc
-- Audit temuan: tidak ada validasi bahwa approver != pemilik dokumen.
-- Jika satu user diberi role WAKA_KURIKULUM sekaligus memiliki dokumen
-- MENUNGGU_WAKA, secara teknis bisa approve dokumennya sendiri.
-- Fix: tambah AND teacher_user_id != auth.uid() di SELECT validasi.
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Dokumen tidak ditemukan, status tidak valid, atau tidak boleh menyetujui dokumen sendiri';
  END IF;
