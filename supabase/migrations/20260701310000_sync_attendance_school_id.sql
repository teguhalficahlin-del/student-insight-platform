-- ============================================================
-- FIX (residual C1) — fn_sync_attendance_batch stamp school_id
-- ============================================================
-- Penerima data absensi offline (sync-attendance-batch → RPC ini)
-- meng-INSERT sync_idempotency (tabel "tanpa induk") TANPA school_id
-- → gagal NOT NULL di service-role. attendance & teacher_attendance_log
-- mewarisi school_id via trigger (dari jadwal), tapi sync_idempotency tak
-- punya induk. Perbaikan: derive school_id dari baris jadwal yang di-lock
-- (v_schedule.school_id) dan set eksplisit. Fondasi Kelompok 6 (offline).
-- ============================================================
...
BEGIN
    ...
    LOOP
        INSERT INTO attendance (
            ...
            recorded_by_user_id, is_void, void_reason
        )
        VALUES (
            p_schedule_id,
    ...
    -- d. Record idempotency key — FIX: stamp school_id dari jadwal.
