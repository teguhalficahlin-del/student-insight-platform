-- Fix: fn_apply_schedule_templates — nonaktifkan teaching_assignments guru lama.
-- Sebelumnya, guru yang dihapus dari schedule_templates masih punya
-- teaching_assignments is_active = true → portal guru masih tampilkan kelas tersebut.
-- Fix: setelah UPSERT assignments baru, UPDATE is_active = false untuk assignment
-- yang teacher_id + class_id-nya sudah tidak ada di schedule_templates periode ini.
...
BEGIN
    ...
    -- Nonaktifkan assignments yang teacher+kelas-nya sudah tidak ada di template
    -- (guru dihapus atau dipindah kelas sejak last apply). Tidak hapus — hanya
    -- is_active = false agar data historis tetap ada.
