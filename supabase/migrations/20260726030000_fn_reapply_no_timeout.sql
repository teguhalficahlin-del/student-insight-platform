-- Fix: fn_reapply_schedule_templates timeout (error 57014) pada free tier.
-- Operasi bulk DELETE + INSERT ribuan sesi bisa melebihi statement timeout.
-- SET LOCAL hanya berlaku dalam transaksi ini, tidak bocor ke koneksi lain.
...
REVOKE EXECUTE ON FUNCTION public.fn_reapply_schedule_templates(text, semester, uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.fn_reapply_schedule_templates(text, semester, uuid) FROM anon;
GRANT  EXECUTE ON FUNCTION public.fn_reapply_schedule_templates(text, semester, uuid) TO authenticated;
