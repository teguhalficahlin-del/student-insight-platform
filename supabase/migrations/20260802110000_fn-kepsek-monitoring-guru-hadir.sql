-- Migration: 20260802110000_fn-kepsek-monitoring-guru-hadir.sql
--
-- Ganti definisi "guru hadir" dari teacher_indicator = 'HADIR' (diset cron tengah malam)
-- menjadi EXISTS(attendance NOT void) — real-time, tidak bergantung cron.
-- Perubahan di 3 lokasi: inner query Lokasi 1 (summary), Lokasi 2 (chart bulanan),
-- Lokasi 3 (chart harian). Semua bagian lain identik dengan 20260802100000.
-- Alias ax dipakai di subquery EXISTS agar tidak ambigu dengan alias a di LEFT JOIN luar.
