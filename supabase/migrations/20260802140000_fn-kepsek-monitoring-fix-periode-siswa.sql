-- Migration: 20260802140000_fn-kepsek-monitoring-fix-periode-siswa.sql
--
-- Fix bug filter periode siswa di Lokasi 1 fn_kepsek_monitoring.
--
-- BUG (20260802130000):
--   Filter periode ada di ON clause LEFT JOIN ts2, tapi CASE WHEN
--   tidak menyebut ts2 — sehingga attendance di luar periode ikut
--   dihitung. Contoh: hari ini (Minggu, tidak ada jadwal) = 376,
--   seharusnya 0.
--
-- FIX:
--   Tambah AND ts2.session_date BETWEEN v_date_start AND v_date_end
--   ke dalam CASE WHEN pada subquery pct_siswa dan siswa_hadir.
--   ts2.session_date IS NULL (LEFT JOIN tidak match) → BETWEEN NULL
--   → NULL → FALSE → student_id tidak dihitung. Ini benar.
--
-- DIVERIFIKASI:
--   Formula baru: hari ini (Minggu) = 0 ✅, 7 hari = 174 ✅
--
-- Perubahan HANYA di dua CASE WHEN subquery siswa (Lokasi 1).
-- pct_guru, guru_hadir, guru_total, siswa_total, count_late,
-- count_exits, Lokasi 2, Lokasi 3 identik dengan 20260802130000.
