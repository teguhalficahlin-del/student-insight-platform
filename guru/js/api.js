...
// ═══════════════════════════════════════════════════════
// PENILAIAN — Learning Objectives (TP)
// ═══════════════════════════════════════════════════════
export async function getLearningObjectives(schoolId, classId, subjectId, academicYear, semester) {
    try {
        const { data, error } = await supabase
            .from('learning_objectives')
            .select('id, kode_tp, deskripsi_tp, urutan, teacher_id')
            .eq('school_id',    schoolId)
            .eq('class_id',     classId)
            .eq('subject_id',   subjectId)
            .eq('academic_year', academicYear)
            ...
            .order('kode_tp', { ascending: true });
        if (error) { console.warn('[penilaian] getLearningObjectives:', error.message); return []; }
        return data ?? [];
    } catch (e) { console.warn('[penilaian] getLearningObjectives exception:', e); return []; }
}

export async function createLearningObjective({ schoolId, teacherId, classId, subjectId, academicYear, semester, kodeTp, deskripsiTp, urutan = 0 }) {
    const { data, error } = await supabase
        .from('learning_objectives')
        .insert({
            school_id: schoolId, teacher_id: teacherId, class_id: classId,
            subject_id: subjectId, academic_year: academicYear, semester,
            kode_tp: kodeTp, deskripsi_tp: deskripsiTp, urutan,
        })
        .select('id').single();
    if (error) throw error;
    return data;
...
export async function updateLearningObjective(id, patch) {
    const { error } = await supabase
        .from('learning_objectives')
...
export async function deleteLearningObjective(id) {
    const { error } = await supabase
        .from('learning_objectives')
        .delete()
        .eq('id', id);
...
// ═══════════════════════════════════════════════════════
// PENILAIAN — Assessments (Asesmen)
// ═══════════════════════════════════════════════════════
export async function getAssessments(schoolId, classId, subjectId, academicYear, semester) {
    try {
        const { data, error } = await supabase
            .from('assessments')
            .select('id, judul, jenis, teknik, tanggal, format_penilaian, tp_id, tindak_lanjut, catatan_tl, is_published, teacher_id')
            .eq('school_id',    schoolId)
            .eq('class_id',     classId)
            .eq('subject_id',   subjectId)
            .eq('academic_year', academicYear)
        ...
        if (error) { console.warn('[penilaian] getAssessments:', error.message); return []; }
        return data ?? [];
    } catch (e) { console.warn('[penilaian] getAssessments exception:', e); return []; }
}

export async function createAssessment({ schoolId, teacherId, classId, subjectId, academicYear, semester, judul, jenis, teknik, tanggal, tpId, formatPenilaian, tindakLanjut, catatanTl }) {
    const { data, error } = await supabase
        .from('assessments')
        .insert({
            school_id: schoolId, teacher_id: teacherId, class_id: classId,
            ...
            tp_id:            tpId            || null,
            format_penilaian: formatPenilaian || null,
            tindak_lanjut:    tindakLanjut    || null,
            catatan_tl:       catatanTl       || null,
        })
        .select('id').single();
    if (error) throw error;
    return data;
